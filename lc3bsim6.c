...
void SR_stage() {
  ...
  if ((!sr_n) && (!sr_z))
    sr_p = 1;
  sr_cc_data = ( (sr_n << 2) + (sr_z << 1) + sr_p) & 0x07;
}
...
void MEM_stage() {
    ...
    /* in: MEMALU.RESULT, DATA.SIZE, MEM.ADDRESS[0]; out: WRITE_WORD */
    /* TODO verify */
    if(Get_DATA_SIZE(uinstr)){
        /* Word */
    ...
    /* BR Logic */
    /* in: IR[11:9], MEM.CC, MEM.V, BR.OP, UNCON.OP, TRAP.OP; out: MEM.PCMUX */
    if(PS.MEM_V){
        if(Get_TRAP_OP(uinstr)){
           MEM_PC_MUX = 2;
        }
        else if(Get_UNCOND_OP(uinstr)){
            MEM_PC_MUX = 1;
        }
        else if(Get_BR_OP(uinstr)){
            int nzp = (PS.MEM_IR >> 9) & 0x07;
            if( (PS.MEM_CC & 0x07) & nzp ){
                MEM_PC_MUX = 1;
            }
            else{
                MEM_PC_MUX = 0;
    ...
    /* Valid latching logic */
    if(PS.MEM_V){
        V_MEM_LD_CC = Get_MEM_LD_CC(uinstr);
        V_MEM_LD_REG = Get_MEM_LD_REG(uinstr);
        v_mem_br_stall = Get_MEM_BR_STALL(uinstr);
    }
    ...
    /* SR Valid (Bubble) Logic */
    /* TODO: Verify */
    NEW_PS.SR_V  = (mem_stall || PS.MEM_V == 0) ? 0 : 1; /* Insert bubble */ 
...
void AGEX_stage() {
    ...
    ADDR1 = (Get_ADDR1MUX(uinstr)) ? PS.AGEX_SR1 : PS.AGEX_NPC;
    switch(Get_ADDR2MUX(uinstr)){
        case 0 : 
            ADDR2 = 0;
            break;
        case 1 : 
    ...
    ADDR2 = (Get_LSHF1(uinstr)) ? ADDR2 << 1 : ADDR2;
    ADDR_ADD = Low16bits(ADDR1 + ADDR2);

    /* ALU Logic */
    SHF = Shifter(PS.AGEX_SR1, ((PS.AGEX_IR >> 4) & 0x01), ((PS. AGEX_IR >> 5) & 0x01), (PS. AGEX_IR & 0x0F) );
    B = (Get_SR2MUX(uinstr)) ? Low16bits(sext( (PS.AGEX_IR & 0x1F), 5) ) : PS.AGEX_SR2; 
    switch(Get_ALUK(uinstr)){
        case 0 : 
            ALU = PS.AGEX_SR1 + B;
            break;
        case 1 : 
    ...
    /* Stall Logic */
    /* TODO */
    LD_MEM = (mem_stall) ? 0 : 1; /* Stall MEM stage */


	if (LD_MEM) {
		/* Your code for latching into MEM latches goes here */
        NEW_PS.MEM_ADDRESS = (Get_ADDRESSMUX(uinstr)) ? ADDR_ADD : (PS.AGEX_IR << 1) & 0x1FF;
        NEW_PS.MEM_NPC = PS.AGEX_NPC;
        NEW_PS.MEM_CC = PS.AGEX_CC;
        NEW_PS.MEM_ALU_RESULT = (Get_ALU_RESULTMUX(uinstr)) ? ALU : SHF;
        NEW_PS.MEM_IR = PS.AGEX_IR;
        NEW_PS.MEM_DRID = PS.AGEX_DRID;
        NEW_PS.MEM_V = PS.AGEX_V; /* Insert bubble into MEM stage */
...
void DE_stage() {
    ...
    /* Internal Stage Signals */
    int SR1_ID = ( (PS.DE_IR >> 6) & 0x07);
    int SR2_ID = 0;
    int opcode = (PS.DE_IR >> 12) & 0x0F;

    SR2_ID = ( (opcode == 3) || (opcode == 7) ) ? (PS.DE_IR >> 9) & 0x07 : PS.DE_IR & 0x07; 
